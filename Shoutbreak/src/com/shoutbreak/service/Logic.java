...
public class Logic {
					...
					case C.HTTP_DID_SUCCEED: {
						...
						try {
							...
							if (code.equals(C.JSON_CODE_PING_OK)) {
								...
								_uiThreadHandler.sendMessage(Message.obtain(_uiThreadHandler, C.STATE_LEVEL_CHANGE, xPacket));
							} else {
				...
				switch (message.what) {
					case C.HTTP_DID_SUCCEED: {			
						try {
							...
							int level = (int) xPacket.json.getLong(C.JSON_LEVEL);
	...
	public void levelChange(Message message) {
		try {
			CrossThreadPacket xPacket = (CrossThreadPacket)message.obj;
			int newLevel = (int)xPacket.json.getLong(C.JSON_LEVEL);
			int newPoints = (int)xPacket.json.getLong(C.JSON_POINTS);
			int nextLevelAt = (int)xPacket.json.getLong(C.JSON_NEXT_LEVEL_AT);
			_user.setLevel(newLevel);
			_user.setPoints(newPoints);
			_user.setNextLevelAt(nextLevelAt);
			xPacket.uiCode = C.UI_LEVEL_CHANGE;
			_uiThreadHandler.sendMessage(Message.obtain(_uiThreadHandler, C.STATE_IDLE, xPacket));
		} catch (JSONException ex) {
			ErrorManager.manage(ex);
		}
