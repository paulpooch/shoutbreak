...
class DBEngine {
	...
	private $LIVE_USERS_TIMEOUT = 1260; // 21 minutes
	...
	public function checkInbox($uid) {
		...
		if ($inbox) {
			...
			foreach ($inbox as $shoutID) {
				$log->LogInfo($uid . " PICKED UP " . $shoutID);
			...
			if ($targets) {
				...
				while(count($targets) > 0 && $moreExist) {
					...
					foreach ($targets as $target) {
						...
						$log->LogInfo("SHOUT HIT " . $targetUID);	
						if ($targetUID == $uid) {
							$wasSenderHit = true;
						}
					...
					if (!$illegalVote) {
						...
						if ($voteWorkedGivePoints) {	
							...
							if ($points) {
								...
								$newPoints = $points + Config::pointsForVote($level);
	...
	public function cron_cullLiveUsers() {
		...
		while(count($timedOutUsers) > 0 && $moreExist) {
			...
			foreach ($timedOutUsers as $user) {
				...
				$log->LogInfo($user['Attributes']['user_id']);	
			}
			if ($this->sdb->NextToken != null) {
				$moreExist = true;
